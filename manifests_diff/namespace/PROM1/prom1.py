...
def get_config():
    namespace = NamespaceConfig(
        ...
        SECMAN=SecManConfig(
            SBER_CA_SERVER_CN="strategy-selection.ci09708620-strategy-selection.apps.a4sxasyr.k8s.ca.sbrf.ru"
        ),
        ISTIO=IstioConfig(
            INGRESS=IngressConfig(
                NETWORKING_MTLS=NetworkingMtlsConfig(
                    strategy_selection=PatternsAgentConfig(
                        HOST="strategy-selection.ci09708620-strategy-selection.apps.a4sxasyr.k8s.ca.sbrf.ru",
                        PORT=5443
                    )
                )
            )
